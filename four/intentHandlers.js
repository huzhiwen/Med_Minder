...
var registerIntentHandlers = function (intentHandlers, skillContext) {
    ...
    intentHandlers.SetUpperBoundIntent = function (intent, session, response) {
        ...
        if (isNaN(upperboundValue)) {
            ...
            response.ask('sorry, I did not hear the upper bound number, could you please say that again', 'please say the number again');
            return;
        }  
        ...
        storage.loadGame(session, function (currentGame) {
            ...
            speechOutput += 'You just assign upper bound value:' + upperboundValue + ' for ' + targetPlayer + '. ';

            if (currentGame.data.players.length == 1 || currentGame.data.players.length > 3) {
                speechOutput += 'Upper bound value is ' + upperboundValue + ' for.' + targetPlayer + 'in total.';
            } else {
                speechOutput += 'That\'s ';
                currentGame.data.players.forEach(function (player, index) {
                    ...
                    speechOutput += player + ', ' + currentGame.data.upperbounds[player];
                    speechOutput += ', ';
                });
            }
            currentGame.save(function () {
    ...
    intentHandlers.AddScoreIntent = function (intent, session, response) {
        ...
        if (isNaN(scoreValue)) {
            ...
            response.ask('sorry, your number is invalid, could you please say that again', 'please say the number again');
            return;
        }
    ...
    intentHandlers.TellScoresIntent = function (intent, session, response) {
        ...
        storage.loadGame(session, function (currentGame) {
            ...
            sortedPlayerScores.forEach(function (playerScore, index) {
                if (index === 0) {
                    speechOutput += playerScore.player  + 'was taken '+ playerScore.score + 'time';
                    if (playerScore.score > 1) {
                        speechOutput += 's';
                    }
                } else if (index === sortedPlayerScores.length - 1) {
                    speechOutput += 'And' + playerScore.player + 'was taken'  + playerScore.score + 'time';
                } else {
